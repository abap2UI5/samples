...
CLASS z2ui5_cl_smp_app_000 IMPLEMENTATION.
  ...
  METHOD get_catalog.
    ...
    result = VALUE #(
      ...
      ( group = `Basic I` header = `Message` sub = `Message Model & Manager (C)` app = `z2ui5_cl_smp_app_467` )
      ...
      ( group = `Basic I` header = `Popover` sub = `Opened with the View Build` app = `z2ui5_cl_smp_app_490` )
      ...
      ( group = `controls - sap.m` header = `ActionListItem` sub = `Use the Action List Item to trigger an action directly from a list` app = `z2ui5_cl_smp_app_216` )
      ...
      ( group = `controls - sap.m` header = `Button` sub = `Buttons trigger user actions and come in a variety of shapes and colors. Placing a button ...` app = `z2ui5_cl_smp_app_259` )
      ( group = `controls - sap.m` header = `Carousel` sub = `A sample of a Carousel that contains images.` app = `z2ui5_cl_smp_app_371` )
      ...
      ( group = `controls - sap.m` header = `ComboBox` sub = `Suggestions wrap automatically when longer then the dropdown width` app = `z2ui5_cl_smp_app_229` )
      ( group = `controls - sap.m` header = `DatePicker` sub = `This example shows different DatePicker value states.` app = `z2ui5_cl_smp_app_294` )
      ( group = `controls - sap.m` header = `DateRangeSelection` sub = `This example shows different DateRangeSelection value states.` app = `z2ui5_cl_smp_app_295` )
      ( group = `controls - sap.m` header = `DateTimePicker` sub = `Value States` app = `z2ui5_cl_smp_app_377` )
      ( group = `controls - sap.m` header = `FeedContent` sub = `Shows the tile containing the text of the feed, a subheader, and a numeric value.` app = `z2ui5_cl_smp_app_275` )
      ...
      ( group = `controls - sap.m` header = `FeedListItem` sub = `This sample shows you how to build a complete feed user interface by combining a ...` app = `z2ui5_cl_smp_app_101` )
      ( group = `controls - sap.m` header = `FlexBox` sub = `Flex Box items can be placed in different areas using the justifyContent and alignItem ...` app = `z2ui5_cl_smp_app_205` )
      ( group = `controls - sap.m` header = `FlexBox` sub = `Flex items can be rendered differently. By default, they are wrapped in a div element ...` app = `z2ui5_cl_smp_app_252` )
      ( group = `controls - sap.m` header = `FlexBox` sub = `In this Flex Box the items are aligned at opposing ends of the container with ...` app = `z2ui5_cl_smp_app_218` )
      ( group = `controls - sap.m` header = `FlexBox` sub = `You can influence the direction and order of elements in horizontal and vertical Flex Box ...` app = `z2ui5_cl_smp_app_245` )
      ( group = `controls - sap.m` header = `FormattedText` sub = `The control can be used for embedding formatted HTML text into your application.` app = `z2ui5_cl_smp_app_015` )
      ...
      ( group = `controls - sap.m` header = `GenericTile` sub = `Shows Feed Tile and News Tile samples that can contain feed content, news content, and a ...` app = `z2ui5_cl_smp_app_278` )
      ( group = `controls - sap.m` header = `GenericTile` sub = `Shows Monitor Tile samples that can contain header, subheader, icon, key value, unit, and ...` app = `z2ui5_cl_smp_app_276` )
      ( group = `controls - sap.m` header = `GenericTile` sub = `Shows the GenericTile while it is loading, if loading fails, and in disabled status.` app = `z2ui5_cl_smp_app_281` )
      ( group = `controls - sap.m` header = `HeaderContainer` sub = `The Header Container with a vertical layout and with divider lines.` app = `z2ui5_cl_smp_app_280` )
      ( group = `controls - sap.m` header = `IconTabBar` sub = `In this example, the Icon Tab Bar is used to apply filters on a table and display the ...` app = `z2ui5_cl_smp_app_368` )
      ( group = `controls - sap.m` header = `IconTabBar` sub = `In this example, the Icon Tab Bar tabs display icons only.` app = `z2ui5_cl_smp_app_221` )
      ( group = `controls - sap.m` header = `IconTabBar` sub = `In this example, the Icon Tab Bar tabs display text and corresponding count.` app = `z2ui5_cl_smp_app_222` )
      ( group = `controls - sap.m` header = `IconTabBar` sub = `In this example, the Icon Tab Bar tabs display text only.` app = `z2ui5_cl_smp_app_224` )
      ( group = `controls - sap.m` header = `IconTabBar` sub = `In this example, the Icon Tab Bar tabs display the text and the count in one line.` app = `z2ui5_cl_smp_app_223` )
      ( group = `controls - sap.m` header = `IconTabBar` sub = `This is an example how to use separators in the Icon Tab Bar. You can choose an icon as a ...` app = `z2ui5_cl_smp_app_225` )
      ( group = `controls - sap.m` header = `IconTabBar` sub = `This sample illustrates nested tabs with or without own content in their root-level tab.` app = `z2ui5_cl_smp_app_226` )
      ( group = `controls - sap.m` header = `IconTabHeader` sub = `Icon Tab Header used standalone, outside of Icon Tab Bar.` app = `z2ui5_cl_smp_app_214` )
      ( group = `controls - sap.m` header = `Image` sub = `Images are faster than words and attract people's attention. Images can also have an ...` app = `z2ui5_cl_smp_app_379` )
      ( group = `controls - sap.m` header = `ImageContent` sub = `Shows ImageContent that can include an icon, a profile image, or a logo with a tooltip.` app = `z2ui5_cl_smp_app_271` )
      ( group = `controls - sap.m` header = `Input` sub = `Input type corresponds to the type attribute of the HTML input tag. On touch devices, it ...` app = `z2ui5_cl_smp_app_210` )
      ( group = `controls - sap.m` header = `Input` sub = `Suggestions wrap automatically when longer then the dropdown width` app = `z2ui5_cl_smp_app_246` )
      ( group = `controls - sap.m` header = `Input` sub = `This sample illustrates the usage of the description with input fields, e.g. description ...` app = `z2ui5_cl_smp_app_251` )
      ( group = `controls - sap.m` header = `Input` sub = `To make sure the password is not shown as clear text you set the 'type' of an input ...` app = `z2ui5_cl_smp_app_213` )
      ( group = `controls - sap.m` header = `InputListItem` sub = `Use the Input List Item on phones to build form like user interfaces.` app = `z2ui5_cl_smp_app_219` )
      ...
      ( group = `controls - sap.m` header = `LightBox` sub = `Displays several image thumbnails. Clicking on each of them will open a LightBox.` app = `z2ui5_cl_smp_app_273` )
      ...
      ( group = `controls - sap.m` header = `MessageView` sub = `A sample with Message View and inside a Dialog and grouping of items` app = `z2ui5_cl_smp_app_038` )
      ( group = `controls - sap.m` header = `MultiComboBox` sub = `` app = `z2ui5_cl_smp_app_140` )
      ( group = `controls - sap.m` header = `MultiComboBox` sub = `Suggestions wrap automatically when longer then the dropdown width` app = `z2ui5_cl_smp_app_233` )
      ( group = `controls - sap.m` header = `MultiInput` sub = `Suggestions wrap automatically when longer then the dropdown width` app = `z2ui5_cl_smp_app_232` )
      ( group = `controls - sap.m` header = `MultiInput` sub = `This sample illustrates the different value states of the sap.m.MultiInput control.` app = `z2ui5_cl_smp_app_267` )
      ( group = `controls - sap.m` header = `NewsContent` sub = `This control is used to display the news content text and subheader in a tile.` app = `z2ui5_cl_smp_app_261` )
      ...
      ( group = `controls - sap.m` header = `NumericContent` sub = `Shows NumericContent including an icon.` app = `z2ui5_cl_smp_app_263` )
      ( group = `controls - sap.m` header = `NumericContent` sub = `Shows NumericContent including numbers, units of measurement, and status arrows ...` app = `z2ui5_cl_smp_app_262` )
      ( group = `controls - sap.m` header = `NumericContent` sub = `This is an example of the NumericContent that contains no margins, so the control is ...` app = `z2ui5_cl_smp_app_228` )
      ( group = `controls - sap.m` header = `ObjectAttribute` sub = `This is an example of Object Attribute used inside Table.` app = `z2ui5_cl_smp_app_302` )
      ( group = `controls - sap.m` header = `ObjectHeader` sub = `An Object Header can set shape of the image by using 'imageShape' property. The shapes ...` app = `z2ui5_cl_smp_app_272` )
      ( group = `controls - sap.m` header = `ObjectListItem` sub = `This sample shows the different states of an Object List Item, which can be set using the ...` app = `z2ui5_cl_smp_app_290` )
      ( group = `controls - sap.m` header = `ObjectMarker` sub = `The ObjectMarker is a small building block representing an object by an icon or text and ...` app = `z2ui5_cl_smp_app_289` )
      ...
      ( group = `controls - sap.m` header = `OverflowToolbar` sub = `OverflowToolbar and Toolbar are often used for left/right alignment. This is easily ...` app = `z2ui5_cl_smp_app_250` )
      ( group = `controls - sap.m` header = `OverflowToolbar` sub = `The sap.m.Title control can be used to place a title inside an OverflowToolbar/Toolbar.` app = `z2ui5_cl_smp_app_217` )
      ( group = `controls - sap.m` header = `Page` sub = `Each screen of a mobile application is typically represented by a 'Page' consisting of a ...` app = `z2ui5_cl_smp_app_227` )
      ( group = `controls - sap.m` header = `Page` sub = `Header, Sub-Header & Footer` app = `z2ui5_cl_smp_app_366` )
      ( group = `controls - sap.m` header = `Panel` sub = `Panels are helpful to group custom content. They can be decorated with header and info ...` app = `z2ui5_cl_smp_app_378` )
      ...
      ( group = `controls - sap.m` header = `SegmentedButton` sub = `Segmented Button used in Input List Item component` app = `z2ui5_cl_smp_app_230` )
      ( group = `controls - sap.m` header = `Select` sub = `Illustrates how the text in items wrap.` app = `z2ui5_cl_smp_app_299` )
      ( group = `controls - sap.m` header = `Select` sub = `Illustrates the usage of a Select in header, footer and content of a page. Note the ...` app = `z2ui5_cl_smp_app_288` )
      ( group = `controls - sap.m` header = `Select` sub = `Illustrates the usage of a Select with icons` app = `z2ui5_cl_smp_app_297` )
      ( group = `controls - sap.m` header = `Select` sub = `Visualizes the validation state of the control, for example, Error, Warning and Success.` app = `z2ui5_cl_smp_app_298` )
      ( group = `controls - sap.m` header = `Slider` sub = `With the Slider a user can choose a value from a numerical range.` app = `z2ui5_cl_smp_app_237` )
      ( group = `controls - sap.m` header = `SlideTile` sub = `Shows Generic Tile with the 2x1 frame type displayed as sliding tiles.` app = `z2ui5_cl_smp_app_274` )
      ( group = `controls - sap.m` header = `SplitContainer` sub = `Master & Detail Pages` app = `z2ui5_cl_smp_app_374` )
      ( group = `controls - sap.m` header = `StandardListItem` sub = `This sample demonstrates the wrapping behavior of the title text and the description ...` app = `z2ui5_cl_smp_app_287` )
      ( group = `controls - sap.m` header = `StepInput` sub = `This example shows different StepInput value states.` app = `z2ui5_cl_smp_app_264` )
      ( group = `controls - sap.m` header = `Switch` sub = `"Some say it is only a switch, I say it is one of the most stylish controls in the ...` app = `z2ui5_cl_smp_app_240` )
      ( group = `controls - sap.m` header = `Text` sub = `The Text control has a property to limit the number of lines for wrapping texts.` app = `z2ui5_cl_smp_app_206` )
      ( group = `controls - sap.m` header = `Text` sub = `with class -Standard Margins - Negative Margins` app = `z2ui5_cl_smp_app_243` )
      ( group = `controls - sap.m` header = `TextArea` sub = `Since 1.38 the growing property of sap.m.TextArea gives the ability of a control to ...` app = `z2ui5_cl_smp_app_236` )
      ( group = `controls - sap.m` header = `TextArea` sub = `This sample illustrates the different value states of the sap.m.TextArea control.` app = `z2ui5_cl_smp_app_234` )
      ( group = `controls - sap.m` header = `TileContent` sub = `Shows the universal container for different content types and context information in the ...` app = `z2ui5_cl_smp_app_241` )
      ( group = `controls - sap.m` header = `TimePicker` sub = `Formats & Steps` app = `z2ui5_cl_smp_app_376` )
      ( group = `controls - sap.m` header = `ToggleButton` sub = `Toggle Buttons can be toggled between pressed and normal state.` app = `z2ui5_cl_smp_app_266` )
      ( group = `controls - sap.m` header = `Toolbar` sub = `Toolbar handles overflow by shrinking items. OverflowToolbar provides an overflow menu ...` app = `z2ui5_cl_smp_app_235` )
      ( group = `controls - sap.uxap` header = `ObjectPageLayout` sub = `Object Page sample showing a layout with subsection titles on top. This is the default ...` app = `z2ui5_cl_smp_app_017` )
      ...
      ( group = `controls - sap.uxap` header = `ObjectPageLayout` sub = `ObjectPage sample with Header Container` app = `z2ui5_cl_smp_app_303` )
      ( group = `controls - sap.f` header = `Card` sub = `This sample illustrates how to specify the predefined header and the content of the Card ...` app = `z2ui5_cl_smp_app_181` )
      ( group = `controls - sap.f` header = `DynamicPage` sub = `Dynamic Page freestyle example with a responsive sap.m.Table in the content area, showing ...` app = `z2ui5_cl_smp_app_030` )
      ( group = `controls - sap.f` header = `GridList` sub = `This sample represents GridList with enabled Drag and Drop functionality.` app = `z2ui5_cl_smp_app_307` )
      ( group = `controls - sap.ui.core` header = `HTML` sub = `With the HTML controls you can easily embed any kind of HTML content into your UI5 mobile ...` app = `z2ui5_cl_smp_app_242` )
      ( group = `controls - sap.ui.core` header = `InvisibleText` sub = `Many controls provide the associations ariaLabelledBy and ariaDescribedBy for ...` app = `z2ui5_cl_smp_app_282` )
      ( group = `controls - sap.ui.layout` header = `Grid` sub = `Split View in different Areas` app = `z2ui5_cl_smp_app_367` )
      ( group = `controls - sap.ui.layout` header = `ResponsiveSplitter` sub = `ResponsiveSplitter is used to visually divide the content of its parent. It consists of ...` app = `z2ui5_cl_smp_app_103` )
      ( group = `controls - sap.ui.layout` header = `Splitter` sub = `Nested Splitter example with 7 content areas` app = `z2ui5_cl_smp_app_260` )
      ( group = `controls - sap.ui.layout` header = `Splitter` sub = `Simple splitter example with three content areas` app = `z2ui5_cl_smp_app_249` )
      ( group = `controls - sap.ui.layout` header = `Splitter` sub = `Simple splitter example with two content areas` app = `z2ui5_cl_smp_app_247` )
      ( group = `controls - sap.ui.layout` header = `Splitter` sub = `Simple splitter example with two content areas that cannot be resized` app = `z2ui5_cl_smp_app_248` )
      ...
      ( group = `controls - sap.ui.codeeditor` header = `CodeEditor` sub = `` app = `z2ui5_cl_smp_app_265` )
