...
CLASS z2ui5_cl_sample_000 IMPLEMENTATION.
  ...
  METHOD get_catalog.
    ...
    result = VALUE #(
      ...
      ( group = `framework` header = `ZZZ` sub = `called by SubApp I` app = `z2ui5_cl_demo_app_126` )
      ( group = `framework` header = `ZZZ` sub = `called by SubApp II` app = `z2ui5_cl_demo_app_132` )
      ( group = `framework` header = `ZZZ` sub = `called by SubApp III` app = `z2ui5_cl_demo_app_184` )
      ( group = `framework` header = `ZZZ` sub = `called by value-help` app = `z2ui5_cl_demo_app_056` )
      ...
      ( group = `controls with cc` header = `History` sub = `` app = `z2ui5_cl_demo_app_139` )
      ( group = `controls with cc` header = `Multi Input` sub = `` app = `z2ui5_cl_demo_app_078` )
      ( group = `controls with cc` header = `Storage` sub = `Store data inside localStorage or sessionStorage` app = `z2ui5_cl_demo_app_327` )
