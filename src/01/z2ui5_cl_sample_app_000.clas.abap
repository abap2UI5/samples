...
CLASS z2ui5_cl_sample_app_000 IMPLEMENTATION.
  ...
  METHOD get_catalog.
    ...
    result = VALUE #(
      ( group = `framework` header = `Binding` sub = `Display dynamic created table RTTI` app = `z2ui5_cl_demo_app_061` )
      ...
      ( group = `framework` header = `Binding IV` sub = `Expression Binding` app = `z2ui5_cl_demo_app_027` )
      ( group = `framework` header = `Binding V` sub = `Use of Integer, Decimals, Dates & Time` app = `z2ui5_cl_demo_app_047` )
      ( group = `framework` header = `Binding VII` sub = `Formatting Currencies` app = `z2ui5_cl_demo_app_067` )
      ( group = `framework` header = `Binding VIII` sub = `SetSizeLimit` app = `z2ui5_cl_demo_app_071` )
      ...
      ( group = `framework` header = `Import View` sub = `Copy & paste views of the UI5 Documentation` app = `z2ui5_cl_demo_app_031` )
      ( group = `framework` header = `Navigation` sub = `Call other apps & exchange data` app = `z2ui5_cl_demo_app_024` )
      ( group = `framework` header = `Popover` sub = `Display with Menu` app = `z2ui5_cl_demo_app_163` )
      ( group = `framework` header = `Popover I` sub = `Simple Example` app = `z2ui5_cl_demo_app_026` )
      ( group = `framework` header = `Popover II` sub = `Item Level of Table` app = `z2ui5_cl_demo_app_052` )
      ( group = `framework` header = `Popover III` sub = `List to select in Popover` app = `z2ui5_cl_demo_app_081` )
      ( group = `framework` header = `Popover IV` sub = `with Quick View` app = `z2ui5_cl_demo_app_109` )
      ( group = `framework` header = `Popup I` sub = `Different ways of calling Popups` app = `z2ui5_cl_demo_app_012` )
      ( group = `framework` header = `Popup II` sub = `Create Popup for Value Help` app = `z2ui5_cl_demo_app_009` )
      ( group = `framework` header = `Popup III` sub = `Popup in Popup - Backend Stack Handling` app = `z2ui5_cl_demo_app_161` )
      ( group = `framework` header = `Standard Margins` sub = `Negative Margins` app = `z2ui5_cl_demo_app_243` )
      ( group = `framework` header = `SubApp I` sub = `Calling another app for rendering` app = `z2ui5_cl_demo_app_117` )
      ( group = `framework` header = `SubApp II` sub = `Use RTTI to render different Subapps` app = `z2ui5_cl_demo_app_131` )
      ( group = `framework` header = `SubApp III` sub = `User Generic Data Refs in Subapps` app = `z2ui5_cl_demo_app_185` )
      ( group = `framework` header = `Templating I` sub = `Basic Example` app = `z2ui5_cl_demo_app_173` )
      ( group = `framework` header = `Templating II` sub = `Nested Views` app = `z2ui5_cl_demo_app_176` )
      ( group = `framework with action` header = `Browser` sub = `Read Infos` app = `z2ui5_cl_demo_app_122` )
      ( group = `framework with action` header = `Clipboard` sub = `Copy & Paste Text` app = `z2ui5_cl_demo_app_325` )
      ( group = `framework with action` header = `Focus I` sub = `Set Focus in Textfield` app = `z2ui5_cl_demo_app_133` )
      ( group = `framework with action` header = `Focus II` sub = `Jump with the focus` app = `z2ui5_cl_demo_app_189` )
      ( group = `framework with action` header = `Keyboard` sub = `Hide/show Soft Keyboard` app = `z2ui5_cl_demo_app_352` )
      ( group = `framework with action` header = `Scroll I` sub = `Scroll to position` app = `z2ui5_cl_demo_app_362` )
      ( group = `framework with action` header = `Scroll II` sub = `Scroll into view` app = `z2ui5_cl_demo_app_363` )
      ( group = `framework with action` header = `Timer I` sub = `Wait n MS and call again the server` app = `z2ui5_cl_demo_app_028` )
      ( group = `framework with action` header = `Timer II` sub = `Set Loading Indicator while Server Request` app = `z2ui5_cl_demo_app_064` )
      ( group = `framework with action` header = `Title` sub = `Set Title` app = `z2ui5_cl_demo_app_125` )
      ( group = `framework with action` header = `URL I` sub = `New Tab Open an URL in a new tab` app = `z2ui5_cl_demo_app_073` )
      ( group = `framework with action` header = `URL II` sub = `Open Telephon, Email usw` app = `z2ui5_cl_demo_app_316` )
      ...
      ( group = `controls with cc` header = `File Uploader I` sub = `` app = `z2ui5_cl_demo_app_074` )
      ( group = `controls with cc` header = `File Uploader II` sub = `` app = `z2ui5_cl_demo_app_075` )
      ( group = `controls with cc` header = `File Uploader III` sub = `` app = `z2ui5_cl_demo_app_136` )
      ( group = `controls with cc` header = `Multi Input` sub = `` app = `z2ui5_cl_demo_app_078` )
      ...
      ( group = `controls` header = `Table Editable` sub = `Set columns editable` app = `z2ui5_cl_demo_app_011` )
      ( group = `controls` header = `Table Selection Modes` sub = `Single Select & Multi Select` app = `z2ui5_cl_demo_app_019` )
