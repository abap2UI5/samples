...
CLASS z2ui5_cl_sample_app_001 IMPLEMENTATION.
  ...
  METHOD get_catalog.
    ...
    result = VALUE #(
      ...
      ( group = `framework - basics` header = `Nested Apps I` sub = `Calling another app for rendering` app = `z2ui5_cl_demo_app_117` )
      ( group = `framework - basics` header = `Nested Apps II` sub = `Use RTTI to render different Subapps` app = `z2ui5_cl_demo_app_131` )
      ( group = `framework - basics` header = `Nested Apps III` sub = `User Generic Data Refs in Subapps` app = `z2ui5_cl_demo_app_185` )
      ...
      ( group = `framework - action` header = `Browser` sub = `Read Infos` app = `z2ui5_cl_demo_app_122` )
