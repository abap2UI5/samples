...
CLASS z2ui5_cl_sample_app_001 IMPLEMENTATION.
  ...
  METHOD get_catalog.
    ...
    result = VALUE #(
      ...
      ( group = `controls - sap.m` header = `sap.m.ActionSheet` sub = `Action Sheet provides an easier way of showing a list of actions and allowing the user to select one. Title and Cancel button can be shown or hidden. Without an icon the entry will be left-aligned (see the last action in the list).` app = `z2ui5_cl_demo_app_373` )
      ...
      ( group = `controls - sap.m` header = `sap.m.BusyIndicator` sub = `The Busy Indicator signals that some operation is going on and that the user must wait. It does not block the current UI screen so other operations could be triggered in parallel.` app = `z2ui5_cl_demo_app_215` )
      ...
      ( group = `controls - sap.m` header = `sap.m.FlexBox` sub = `Flex items can be rendered differently. By default, they are wrapped in a div element. Optionally, the bare controls can be rendered directly. This can affect the resulting layout.` app = `z2ui5_cl_demo_app_252` )
      ...
      ( group = `controls - sap.m` header = `sap.m.Input` sub = `Input type corresponds to the type attribute of the HTML input tag. On touch devices, it controls the keyboard layout. On desktop, the effect of this setting is browser dependent.` app = `z2ui5_cl_demo_app_210` )
      ...
      ( group = `controls - sap.m` header = `sap.m.MaskInput` sub = `The sap.m.MaskInput control allows users to easily enter data in a certain format and in a fixed- width input (for example: date, time, credit card number, and others).` app = `z2ui5_cl_demo_app_110` )
      ...
      ( group = `controls - sap.m` header = `sap.m.NumericContent` sub = `Shows NumericContent including numbers, units of measurement, and status arrows indicating a trend. The numbers can be colored according to their meaning.` app = `z2ui5_cl_demo_app_262` )
      ...
      ( group = `controls - sap.m` header = `sap.m.Page` sub = `Each screen of a mobile application is typically represented by a 'Page' consisting of a header, a scrollable content area and optionally a footer. The standard header offers a navigation button and a title. Alternatively you can provide a customer header. Gernerally you should use Toolbars in the Page. If you need a centered title you may use a Bar.` app = `z2ui5_cl_demo_app_227` )
      ...
      ( group = `controls - sap.m` header = `sap.m.RadioButton` sub = `Typically the Radio Button is used by other controls. E.g. the List uses it for the single selection. But you can also use the Radio Buttons control directly, to allow selection of exactly one of multiple options.` app = `z2ui5_cl_demo_app_207` )
      ...
      ( group = `controls - sap.m` header = `sap.m.StandardListItem` sub = `This sample demonstrates the wrapping behavior of the title text and the description text. In desktop mode, the character limit is set to 300 characters, whereas in the phone mode, the character limit is set to 100 characters.` app = `z2ui5_cl_demo_app_287` )
      ...
      ( group = `controls - sap.uxap` header = `sap.uxap.ObjectPageLayout` sub = `Object Page sample showing a layout with subsection titles on top. This is the default layout. The sample also shows the 'Edit header' button in the Header Content area.` app = `z2ui5_cl_demo_app_017` )
      ...
      ( group = `controls - sap.f` header = `sap.f.DynamicPage` sub = `Dynamic Page freestyle example with a responsive sap.m.Table in the content area, showing that each control can be placed in the title and the header content areas.` app = `z2ui5_cl_demo_app_030` )
