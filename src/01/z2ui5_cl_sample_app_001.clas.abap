...
CLASS z2ui5_cl_sample_app_001 IMPLEMENTATION.
  ...
  METHOD get_catalog.
    ...
    result = VALUE #(
      ...
      ( group = `Basic I` header = `Model` sub = `Device Model in views and popups` app = `z2ui5_cl_demo_app_445` )
      ...
      ( group = `Basic II` header = `Browser` sub = `Title` app = `z2ui5_cl_demo_app_125` )
      ( group = `Basic II` header = `Focus I` sub = `Set Focus in Textfield` app = `z2ui5_cl_demo_app_133` )
      ( group = `Basic II` header = `Focus II` sub = `Jump with the focus` app = `z2ui5_cl_demo_app_189` )
      ( group = `Basic II` header = `Input` sub = `Clipboard` app = `z2ui5_cl_demo_app_325` )
      ( group = `Basic II` header = `Input` sub = `Hide/show Soft Keyboard` app = `z2ui5_cl_demo_app_352` )
      ( group = `Basic II` header = `List` sub = `Events & Visualization` app = `z2ui5_cl_demo_app_048` )
      ( group = `Basic II` header = `List` sub = `Filter and sort via backend event (A)` app = `z2ui5_cl_demo_app_454` )
      ( group = `Basic II` header = `List` sub = `Live filter without backend (A)` app = `z2ui5_cl_demo_app_455` )
      ...
      ( group = `Basic II` header = `Scroll` sub = `Scroll into view` app = `z2ui5_cl_demo_app_363` )
      ( group = `Basic II` header = `Scroll` sub = `Scroll to position` app = `z2ui5_cl_demo_app_362` )
      ...
      ( group = `Basic II` header = `Table` sub = `Search Backend` app = `z2ui5_cl_demo_app_053` )
      ( group = `Basic II` header = `Table` sub = `Search Backend Live` app = `z2ui5_cl_demo_app_059` )
      ...
      ( group = `Basic II` header = `Timer` sub = `Loading Indicator with WAIT UP Backend` app = `z2ui5_cl_demo_app_064` )
      ( group = `Basic II` header = `Timer` sub = `Wait n MS and call again the server` app = `z2ui5_cl_demo_app_028` )
      ( group = `Basic II` header = `Tree` sub = `Drag and Drop` app = `z2ui5_cl_demo_app_461` )
      ...
      ( group = `Basic II` header = `ui.Table` sub = `Default Filtering` app = `z2ui5_cl_demo_app_143` )
      ...
      ( group = `Basic II` header = `URL` sub = `New Tab Open an URL in a new tab` app = `z2ui5_cl_demo_app_073` )
      ( group = `Basic II` header = `URL` sub = `Open Telephon, Email usw` app = `z2ui5_cl_demo_app_316` )
