...
CLASS z2ui5_cl_sample_app_001 IMPLEMENTATION.
  ...
  METHOD get_catalog.
    ...
    result = VALUE #(
      ...
      ( group = `framework - basics` header = `Import View` sub = `Copy & paste views of the UI5 Documentation` app = `z2ui5_cl_demo_app_031` )
      ( group = `framework - basics` header = `Navigation` sub = `Call other apps & exchange data` app = `z2ui5_cl_demo_app_024` )
      ...
      ( group = `framework - basics` header = `Standard Margins` sub = `Negative Margins` app = `z2ui5_cl_demo_app_243` )
      ( group = `framework - basics` header = `SubApp I` sub = `Calling another app for rendering` app = `z2ui5_cl_demo_app_117` )
      ( group = `framework - basics` header = `SubApp II` sub = `Use RTTI to render different Subapps` app = `z2ui5_cl_demo_app_131` )
      ( group = `framework - basics` header = `SubApp III` sub = `User Generic Data Refs in Subapps` app = `z2ui5_cl_demo_app_185` )
      ...
      ( group = `framework - frontend with action` header = `Browser` sub = `Read Infos` app = `z2ui5_cl_demo_app_122` )
      ( group = `framework - frontend with action` header = `Clipboard` sub = `Copy & Paste Text` app = `z2ui5_cl_demo_app_325` )
      ( group = `framework - frontend with action` header = `Focus I` sub = `Set Focus in Textfield` app = `z2ui5_cl_demo_app_133` )
      ( group = `framework - frontend with action` header = `Focus II` sub = `Jump with the focus` app = `z2ui5_cl_demo_app_189` )
      ( group = `framework - frontend with action` header = `Keyboard` sub = `Hide/show Soft Keyboard` app = `z2ui5_cl_demo_app_352` )
      ( group = `framework - frontend with action` header = `Scroll I` sub = `Scroll to position` app = `z2ui5_cl_demo_app_362` )
      ( group = `framework - frontend with action` header = `Scroll II` sub = `Scroll into view` app = `z2ui5_cl_demo_app_363` )
      ( group = `framework - frontend with action` header = `Timer I` sub = `Wait n MS and call again the server` app = `z2ui5_cl_demo_app_028` )
      ( group = `framework - frontend with action` header = `Timer II` sub = `Set Loading Indicator while Server Request` app = `z2ui5_cl_demo_app_064` )
      ( group = `framework - frontend with action` header = `Title` sub = `Set Title` app = `z2ui5_cl_demo_app_125` )
      ( group = `framework - frontend with action` header = `URL I` sub = `New Tab Open an URL in a new tab` app = `z2ui5_cl_demo_app_073` )
      ( group = `framework - frontend with action` header = `URL II` sub = `Open Telephon, Email usw` app = `z2ui5_cl_demo_app_316` )
      ( group = `framework - controls with action` header = `MessageBox I` sub = `Display different types` app = `z2ui5_cl_demo_app_008` )
      ( group = `framework - controls with action` header = `MessageToast` sub = `Ouput different abap types` app = `z2ui5_cl_demo_app_187` )
      ( group = `framework - controls with action` header = `Nav Container I` sub = `` app = `z2ui5_cl_demo_app_088` )
      ( group = `framework - controls with action` header = `Nested Views I` sub = `Basic Example` app = `z2ui5_cl_demo_app_065` )
      ( group = `framework - controls with action` header = `Nested Views II` sub = `Head & Item Table` app = `z2ui5_cl_demo_app_097` )
      ( group = `framework - controls with action` header = `Nested Views III` sub = `Head & Item Table & Detail` app = `z2ui5_cl_demo_app_098` )
      ( group = `framework - controls with action` header = `Nested Views IV` sub = `Sub-App` app = `z2ui5_cl_demo_app_104` )
      ( group = `framework - controls with action` header = `Wizard Control I` sub = `` app = `z2ui5_cl_demo_app_175` )
      ( group = `framework - controls with action` header = `Wizard Control II` sub = `Next step & SubSequentStep` app = `z2ui5_cl_demo_app_202` )
      ( group = `controls - extended with cc` header = `CameraSelector` sub = `` app = `z2ui5_cl_demo_app_306` )
      ( group = `controls - extended with cc` header = `Data loss protection` sub = `` app = `z2ui5_cl_demo_app_279` )
      ( group = `controls - extended with cc` header = `File Uploader I` sub = `` app = `z2ui5_cl_demo_app_074` )
      ( group = `controls - extended with cc` header = `File Uploader II` sub = `` app = `z2ui5_cl_demo_app_075` )
      ( group = `controls - extended with cc` header = `File Uploader III` sub = `` app = `z2ui5_cl_demo_app_136` )
      ( group = `controls - extended with cc` header = `Multi Input` sub = `` app = `z2ui5_cl_demo_app_078` )
