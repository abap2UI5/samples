...
CLASS z2ui5_cl_sample_app_001 IMPLEMENTATION.
  ...
  METHOD get_catalog.
    ...
    result = VALUE #(
      ...
      ( group = `controls` header = `sap.f.Card` sub = `This sample illustrates how to specify the predefined header and the content of the Card control.` app = `z2ui5_cl_demo_app_181` )
      ( group = `controls` header = `sap.f.DynamicPage` sub = `Dynamic Page freestyle example with a responsive sap.m.Table in the content area, showing that each control can be placed in the title and the header content areas.` app = `z2ui5_cl_demo_app_030` )
      ( group = `controls` header = `sap.f.GridList` sub = `This sample represents GridList with enabled Drag and Drop functionality.` app = `z2ui5_cl_demo_app_307` )
      ( group = `controls` header = `sap.m.ActionListItem` sub = `Use the Action List Item to trigger an action directly from a list` app = `z2ui5_cl_demo_app_216` )
      ( group = `controls` header = `sap.m.ActionSheet` sub = `Action Sheet provides an easier way of showing a list of actions and allowing the user to select one. Title and Cancel button can be shown or hidden. Without an icon the entry will be left-aligned (see the last action in the list).` app = `z2ui5_cl_demo_app_373` )
      ...
      ( group = `controls` header = `sap.m.BusyIndicator` sub = `The Busy Indicator signals that some operation is going on and that the user must wait. It does not block the current UI screen so other operations could be triggered in parallel.` app = `z2ui5_cl_demo_app_215` )
      ...
      ( group = `controls` header = `sap.m.Carousel` sub = `A sample of a Carousel that contains images.` app = `z2ui5_cl_demo_app_371` )
      ...
      ( group = `controls` header = `sap.m.ComboBox` sub = `Suggestions wrap automatically when longer then the dropdown width` app = `z2ui5_cl_demo_app_229` )
      ...
      ( group = `controls` header = `sap.m.FeedInput` sub = `This sample shows a standalone feed input with different settings.` app = `z2ui5_cl_demo_app_283` )
      ( group = `controls` header = `sap.m.FeedListItem` sub = `This sample shows you how to build a complete feed user interface by combining a FeedInput with a list of FeedListItems.` app = `z2ui5_cl_demo_app_101` )
      ( group = `controls` header = `sap.m.FlexBox` sub = `Flex Box items can be placed in different areas using the justifyContent and alignItem properties.` app = `z2ui5_cl_demo_app_205` )
      ...
      ( group = `controls` header = `sap.m.FlexBox` sub = `In this Flex Box the items are aligned at opposing ends of the container with justifyContent set to 'SpaceBetween'.` app = `z2ui5_cl_demo_app_218` )
      ...
      ( group = `controls` header = `sap.m.FormattedText` sub = `The control can be used for embedding formatted HTML text into your application.` app = `z2ui5_cl_demo_app_015` )
      ...
      ( group = `controls` header = `sap.m.GenericTile` sub = `Shows Feed Tile and News Tile samples that can contain feed content, news content, and a footer.` app = `z2ui5_cl_demo_app_278` )
      ...
      ( group = `controls` header = `sap.m.GenericTile` sub = `Shows the GenericTile while it is loading, if loading fails, and in disabled status.` app = `z2ui5_cl_demo_app_281` )
      ...
      ( group = `controls` header = `sap.m.IconTabBar` sub = `In this example, the Icon Tab Bar is used to apply filters on a table and display the count of the items for each view.` app = `z2ui5_cl_demo_app_368` )
      ( group = `controls` header = `sap.m.IconTabBar` sub = `In this example, the Icon Tab Bar tabs display icons only.` app = `z2ui5_cl_demo_app_221` )
      ( group = `controls` header = `sap.m.IconTabBar` sub = `In this example, the Icon Tab Bar tabs display text and corresponding count.` app = `z2ui5_cl_demo_app_222` )
      ( group = `controls` header = `sap.m.IconTabBar` sub = `In this example, the Icon Tab Bar tabs display text only.` app = `z2ui5_cl_demo_app_224` )
      ( group = `controls` header = `sap.m.IconTabBar` sub = `In this example, the Icon Tab Bar tabs display the text and the count in one line.` app = `z2ui5_cl_demo_app_223` )
      ( group = `controls` header = `sap.m.IconTabBar` sub = `This is an example how to use separators in the Icon Tab Bar. You can choose an icon as a separator or use the default vertical line.` app = `z2ui5_cl_demo_app_225` )
      ( group = `controls` header = `sap.m.IconTabBar` sub = `This sample illustrates nested tabs with or without own content in their root-level tab.` app = `z2ui5_cl_demo_app_226` )
      ( group = `controls` header = `sap.m.IconTabHeader` sub = `Icon Tab Header used standalone, outside of Icon Tab Bar.` app = `z2ui5_cl_demo_app_214` )
      ( group = `controls` header = `sap.m.Image` sub = `Images are faster than words and attract people's attention. Images can also have an active state or be used in SVG format.` app = `z2ui5_cl_demo_app_379` )
      ( group = `controls` header = `sap.m.ImageContent` sub = `Shows ImageContent that can include an icon, a profile image, or a logo with a tooltip.` app = `z2ui5_cl_demo_app_271` )
      ( group = `controls` header = `sap.m.Input` sub = `Input type corresponds to the type attribute of the HTML input tag. On touch devices, it controls the keyboard layout. On desktop, the effect of this setting is browser dependent.` app = `z2ui5_cl_demo_app_210` )
      ...
      ( group = `controls` header = `sap.m.Input` sub = `To make sure the password is not shown as clear text you set the 'type' of an input control to 'Password'.` app = `z2ui5_cl_demo_app_213` )
      ( group = `controls` header = `sap.m.InputListItem` sub = `Use the Input List Item on phones to build form like user interfaces.` app = `z2ui5_cl_demo_app_219` )
      ( group = `controls` header = `sap.m.Label` sub = `Labels are helpful when you need to describe some other UI control.` app = `z2ui5_cl_demo_app_051` )
      ( group = `controls` header = `sap.m.LightBox` sub = `Displays several image thumbnails. Clicking on each of them will open a LightBox.` app = `z2ui5_cl_demo_app_273` )
      ...
      ( group = `controls` header = `sap.m.MaskInput` sub = `The sap.m.MaskInput control allows users to easily enter data in a certain format and in a fixed- width input (for example: date, time, credit card number, and others).` app = `z2ui5_cl_demo_app_110` )
      ( group = `controls` header = `sap.m.MenuButton` sub = `This control is used to open a menu in both desktop and mobile.` app = `z2ui5_cl_demo_app_372` )
      ...
      ( group = `controls` header = `sap.m.MessageView` sub = `A sample with Message View and inside a Dialog and grouping of items` app = `z2ui5_cl_demo_app_038` )
      ( group = `controls` header = `sap.m.MultiComboBox` sub = `Suggestions wrap automatically when longer then the dropdown width` app = `z2ui5_cl_demo_app_233` )
      ( group = `controls` header = `sap.m.MultiInput` sub = `Suggestions wrap automatically when longer then the dropdown width` app = `z2ui5_cl_demo_app_232` )
      ...
      ( group = `controls` header = `sap.m.NewsContent` sub = `This control is used to display the news content text and subheader in a tile.` app = `z2ui5_cl_demo_app_261` )
      ( group = `controls` header = `sap.m.NotificationListItem` sub = `A list item suitable for showing notifications to the user.` app = `z2ui5_cl_demo_app_375` )
      ...
      ( group = `controls` header = `sap.m.NumericContent` sub = `This is an example of the NumericContent that contains no margins, so the control is aligned to the left and to the top without any margins.` app = `z2ui5_cl_demo_app_228` )
      ...
      ( group = `controls` header = `sap.m.OverflowToolbar` sub = `The sap.m.Title control can be used to place a title inside an OverflowToolbar/Toolbar.` app = `z2ui5_cl_demo_app_217` )
      ( group = `controls` header = `sap.m.Page` sub = `Each screen of a mobile application is typically represented by a 'Page' consisting of a header, a scrollable content area and optionally a footer. The standard header offers a navigation button and a title. Alternatively you can provide a customer header. Gernerally you should use Toolbars in the Page. If you need a centered title you may use a Bar.` app = `z2ui5_cl_demo_app_227` )
      ( group = `controls` header = `sap.m.Panel` sub = `Panels are helpful to group custom content. They can be decorated with header and info toolbars.` app = `z2ui5_cl_demo_app_378` )
      ( group = `controls` header = `sap.m.PlanningCalendar` sub = `PlanningCalendar with single row selection that illustrates the built-in views.` app = `z2ui5_cl_demo_app_080` )
      ( group = `controls` header = `sap.m.ProgressIndicator` sub = `Shows the progress of a process in a graphical way. To indicate the progress, the inside of the ProgressIndicator is filled with a color.` app = `z2ui5_cl_demo_app_022` )
      ( group = `controls` header = `sap.m.RadioButton` sub = `Typically the Radio Button is used by other controls. E.g. the List uses it for the single selection. But you can also use the Radio Buttons control directly, to allow selection of exactly one of multiple options.` app = `z2ui5_cl_demo_app_207` )
      ( group = `controls` header = `sap.m.RadioButtonGroup` sub = `A wrapper for a group of radio buttons.` app = `z2ui5_cl_demo_app_208` )
      ( group = `controls` header = `sap.m.RatingIndicator` sub = `A Rating Indicator can be used to both indicate and/or rate content.` app = `z2ui5_cl_demo_app_220` )
      ...
      ( group = `controls` header = `sap.m.SegmentedButton` sub = `Segmented Button used in Input List Item component` app = `z2ui5_cl_demo_app_230` )
      ...
      ( group = `controls` header = `sap.m.Slider` sub = `With the Slider a user can choose a value from a numerical range.` app = `z2ui5_cl_demo_app_237` )
      ( group = `controls` header = `sap.m.SlideTile` sub = `Shows Generic Tile with the 2x1 frame type displayed as sliding tiles.` app = `z2ui5_cl_demo_app_274` )
      ...
      ( group = `controls` header = `sap.m.Text` sub = `The Text control has a property to limit the number of lines for wrapping texts.` app = `z2ui5_cl_demo_app_206` )
      ( group = `controls` header = `sap.m.TextArea` sub = `Since 1.38 the growing property of sap.m.TextArea gives the ability of a control to automatically grow and shrink dynamically with its content.` app = `z2ui5_cl_demo_app_236` )
      ( group = `controls` header = `sap.m.TextArea` sub = `This sample illustrates the different value states of the sap.m.TextArea control.` app = `z2ui5_cl_demo_app_234` )
      ...
      ( group = `controls` header = `sap.m.ToggleButton` sub = `Toggle Buttons can be toggled between pressed and normal state.` app = `z2ui5_cl_demo_app_266` )
      ( group = `controls` header = `sap.m.Toolbar` sub = `Toolbar handles overflow by shrinking items. OverflowToolbar provides an overflow menu. Bar is able to perfectly center a text if nothing overflows.` app = `z2ui5_cl_demo_app_235` )
      ( group = `controls` header = `sap.tnt.InfoLabel` sub = `InfoLabel with all available color schemes` app = `z2ui5_cl_demo_app_209` )
      ( group = `controls` header = `sap.ui.codeeditor.CodeEditor` sub = `` app = `z2ui5_cl_demo_app_265` )
      ...
      ( group = `controls` header = `sap.uxap.ObjectPageLayout` sub = `Object Page sample showing a layout with subsection titles on top. This is the default layout. The sample also shows the 'Edit header' button in the Header Content area.` app = `z2ui5_cl_demo_app_017` )
      ( group = `controls` header = `sap.uxap.ObjectPageLayout` sub = `ObjectPage sample that demonstrates the combination of header facets and showTitle properties of sections and subsections.` app = `z2ui5_cl_demo_app_330` )
      ( group = `controls` header = `sap.uxap.ObjectPageLayout` sub = `ObjectPage sample with Header Container` app = `z2ui5_cl_demo_app_303` ) ).
