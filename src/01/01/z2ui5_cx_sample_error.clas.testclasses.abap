...
CLASS ltcl_unit_test IMPLEMENTATION.
  METHOD test_raise.
    ...
    TRY.
      ...
      CATCH z2ui5_cx_sample_error INTO DATA(lx).
  ...
  METHOD test_raise_with_prev.
    ...
    DATA(lx_prev) = NEW z2ui5_cx_sample_error( val = `previous error` ).
  ...
  METHOD test_chain_texts.
    ...
    DATA(lx_inner) = NEW z2ui5_cx_sample_error( val = `inner` ).
    DATA(lx_middle) = NEW z2ui5_cx_sample_error( val      = `middle`
                                                previous = lx_inner ).
    DATA(lx_outer) = NEW z2ui5_cx_sample_error( val      = `outer`
                                               previous = lx_middle ).
