...
CLASS z2ui5_cl_sample_000 IMPLEMENTATION.
  ...
  METHOD get_catalog.
    ...
    result = VALUE #(
      ...
      ( group = `Framework` section = `Action` header = `Basic` sub = `Toast, Box & Strip` app = `z2ui5_cl_demo_app_008` )
      ( group = `Framework` section = `Action` header = `Timer I` sub = `Wait n MS and call again the server` app = `z2ui5_cl_demo_app_028` )
      ( group = `Framework` section = `Action` header = `Change CSS` sub = `Send your own CSS to the frontend` app = `z2ui5_cl_demo_app_050` )
      ( group = `Framework` section = `Action` header = `Download CSV` sub = `Export Table as CSV` app = `z2ui5_cl_demo_app_057` )
      ( group = `Framework` section = `Action` header = `Timer II` sub = `Set Loading Indicator while Server Request` app = `z2ui5_cl_demo_app_064` )
      ( group = `Framework` section = `Action` header = `New Tab` sub = `Open an URL in a new tab` app = `z2ui5_cl_demo_app_073` )
