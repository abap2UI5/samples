...
CLASS Z2UI5_CL_DEMO_APP_147 IMPLEMENTATION.
 ...
 METHOD on_rendering.
    ...
    DATA(lo_view_nested2) = z2ui5_cl_xml_view=>factory(
 ...
 METHOD z2ui5_if_app~main.
    ...
    CASE client->get( )-event.
      ...
      WHEN 'OPEN_POPOVER_FROM_POPUP'.
        DATA(lr_view_popup_popover) = z2ui5_cl_xml_view=>factory_popup( ).

* ---------- Create popover window ----------------------------------------------------------------
        DATA(lr_popover_popup) = lr_view_popup_popover->popover( placement = 'Right'
                                             showheader = abap_false
                                             class = `sapUiContentPadding` ).

* ---------- Create vertical box ------------------------------------------------------------------
        DATA(lr_vbox_popup_popover) = lr_popover_popup->vbox(  ).
...
* ---------- Return xml ---------------------------------------------------------------------------
      ...
      WHEN 'SHOW POPOVER_IN_POPUP'.
        ...
        DATA(lr_view_popup) = z2ui5_cl_xml_view=>factory_popup( ).
        DATA(lr_popup) = lr_view_popup->dialog( ).
...
* ---------- Create View --------------------------------------------------------------------------
        DATA(lr_view_nest2) = z2ui5_cl_xml_view=>factory_popup( ).

* ---------- Create popover window ----------------------------------------------------------------
        DATA(lr_popover_nest2) = lr_view_nest2->popover( placement = 'Right'
                                             showheader = abap_false
                                             class = `sapUiContentPadding` ).

* ---------- Create vertical box ------------------------------------------------------------------
        DATA(lr_vbox_nest2) = lr_popover_nest2->vbox(  ).
