...
CLASS z2ui5_cl_smp_app_000 IMPLEMENTATION.
  ...
  METHOD view_display.
    ...
    DATA tile LIKE LINE OF t_catalog.
      ...
      DATA tenths TYPE z2ui5_cl_smp_app_000=>ty_s_block-width.
  ...
  METHOD get_catalog.
    ...
    temp12-group = `Talk to the user`.
    temp12-header = `Menu`.
    temp12-sub = `Menu as Popover with core:require`.
    temp12-keywords = `menu menuitem popover button messagetoast require module`.
    temp12-path = `src`.
    ...
    temp12-group = `Talk to the user`.
    temp12-header = `Popup`.
    temp12-sub = `Edit a Row in a Second App`.
    temp12-keywords = `table edit row popup app nav_app_call get_app_prev add delete save master detail`.
    temp12-path = `src`.
    temp12-app = `z2ui5_cl_smp_app_500`.
    INSERT temp12 INTO TABLE temp11.
    temp12-group = `Talk to the user`.
    temp12-header = `Popup`.
    ...
    temp12-group = `Move through the app`.
    temp12-header = `Hash`.
    temp12-sub = `Routing Mode FRESH (a New Instance)`.
    ...
    temp12-group = `Move through the app`.
    temp12-header = `Hash`.
    temp12-sub = `Routing Mode KEEP (Back to the State)`.
    ...
    temp12-group = `Reach outside the view`.
    temp12-header = `Browser`.
    temp12-sub = `Scan Field with Submit (InputExt) (A,C)`.
    temp12-keywords = `barcode scanner scan field submit enter soft keyboard none inputext focus warehouse`.
    temp12-path = `src`.
    temp12-app = `z2ui5_cl_smp_app_530`.
    INSERT temp12 INTO TABLE temp11.
    temp12-group = `Reach outside the view`.
    temp12-header = `Browser`.
