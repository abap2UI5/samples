...
CLASS z2ui5_cl_smp_app_000 IMPLEMENTATION.
  ...
  METHOD get_catalog.
    ...
    result = VALUE #(
      ( group = `Start here` header = `Basics I`
        sub = `Hello World, the Smallest App`
        keywords = `hello world smallest first app minimal start here template`
        intro = `The ones to read first, in order. One class, one view, one roundtrip - the shape every other sample in this repository grows from, plus the developer tools you will open when something does not render.`
        path = `src` app = `z2ui5_cl_smp_app_493` )
      ( group = `Start here` header = `Basics II` sub = `Data Binding: Input and Button` keywords = `binding _bind model attribute value input button roundtrip messagebox serialize` path = `src` app = `z2ui5_cl_smp_app_494` )
      ( group = `Start here` header = `Basics III` sub = `Lifecycle: Init, Event, Navigated` keywords = `lifecycle roundtrip main dispatcher state serialize check_on_init check_on_event check_on_navigated` path = `src` app = `z2ui5_cl_smp_app_495` )
      ( group = `Start here` header = `Basics IV` sub = `Events, Views and Roundtrips` keywords = `roundtrip restart second view uncaught error controller basics check_on_navigated get` path = `src` app = `z2ui5_cl_smp_app_004` )
      ( group = `Start here` header = `Basics V`
        ...
        path = `src` app = `z2ui5_cl_smp_app_496` )
      ( group = `Start here` header = `Basics VI` sub = `Unit Tests for the App Logic` keywords = `unit test abapunit testclasses assert testable logic method` path = `src` app = `z2ui5_cl_smp_app_503` )
      ( group = `Start here` header = `Basics VII` sub = `Translatable Texts (Text Elements)` keywords = `translation i18n text element text symbol textpool language message class multi language` path = `src` app = `z2ui5_cl_smp_app_519` )
      ...
      ( group = `Get your data on screen` header = `Binding` sub = `Currency Amounts (sap.ui.model.type.Currency)` keywords = `amount decimals leading zeros number format` path = `src` app = `z2ui5_cl_smp_app_067` )
      ( group = `Get your data on screen` header = `Binding` sub = `Dynamic Table Typed at Runtime (RTTI)` keywords = `generic data reference create data ddic dynamic itab` path = `src` app = `z2ui5_cl_smp_app_061` )
      ( group = `Get your data on screen` header = `Binding` sub = `Expression Binding, Types and Composite Parts` keywords = `formatter parts conditional regexp visible enabled syntax` path = `src` app = `z2ui5_cl_smp_app_027` )
      ( group = `Get your data on screen` header = `Binding` sub = `Model setSizeLimit for Large Tables (A)` keywords = `combobox jsonmodel size limit large itab 100 entries` path = `src` app = `z2ui5_cl_smp_app_071` )
      ( group = `Get your data on screen` header = `Binding`
        ...
        path = `src` app = `z2ui5_cl_smp_app_507` )
      ( group = `Get your data on screen` header = `Binding` sub = `Path Only (_bind_path)` keywords = `binding path only bare path _bind_path expression binding sorter binding info composed raw string` path = `src` app = `z2ui5_cl_smp_app_508` )
      ( group = `Get your data on screen` header = `Binding` sub = `Pre-serialized JSON (json)` keywords = `binding json string spliced model node object array pre-serialized _bind json = abap_true no abap type` path = `src` app = `z2ui5_cl_smp_app_509` )
      ( group = `Get your data on screen` header = `Binding` sub = `Single Table Cell (tab_index)` keywords = `cell input internal table row field level` path = `src` app = `z2ui5_cl_smp_app_144` )
      ( group = `Get your data on screen` header = `Binding` sub = `Structure Fields and INCLUDEs` keywords = `structure component include flat form level` path = `src` app = `z2ui5_cl_smp_app_166` )
      ( group = `Get your data on screen` header = `Binding` sub = `Types for Integer, Decimal, Date and Time` keywords = `type conversion sum amount number field` path = `src` app = `z2ui5_cl_smp_app_047` )
      ( group = `Get your data on screen` header = `Formatter` sub = `ABAP Date and Time Strings (DATS/TIMS)` keywords = `dats tims conversion initial date 00000000 sy-datum` path = `src` app = `z2ui5_cl_smp_app_450` )
      ( group = `Get your data on screen` header = `Formatter` sub = `Date Object for the DatePicker` keywords = `datepicker datevalue javascript date object iso` path = `src` app = `z2ui5_cl_smp_app_457` )
      ...
      ( group = `Show many rows` header = `Table` sub = `Large Table with Growing and ScrollContainer` keywords = `growing 10000 rows sticky toolbar sort performance` path = `src` app = `z2ui5_cl_smp_app_006` )
      ( group = `Show many rows` header = `Table` sub = `Live Search over a Large Table` keywords = `live search table filter keystroke roundtrip busy indicator overlay check_queue_last check_no_busy typing` path = `src` app = `z2ui5_cl_smp_app_059` )
      ( group = `Show many rows` header = `Table`
        ...
        path = `src` app = `z2ui5_cl_smp_app_504` )
      ( group = `Show many rows` header = `Table` sub = `Search in the Backend (SearchField)` keywords = `search go enter server side where` path = `src` app = `z2ui5_cl_smp_app_053` )
      ( group = `Show many rows` header = `Table` sub = `Selection Modes: Single and Multi Select` keywords = `selectionmode none single multi segmentedbutton checkbox` path = `src` app = `z2ui5_cl_smp_app_019` )
      ( group = `Show many rows` header = `Tree` sub = `Drag and Drop Nodes (A,C)` keywords = `dnd move node hierarchy binding context` path = `src` app = `z2ui5_cl_smp_app_461` )
      ...
      ( group = `Talk to the user` header = `Event` sub = `Link with preventDefault (A)` keywords = `link href default action check_prevent_default` path = `src` app = `z2ui5_cl_smp_app_472` )
      ( group = `Talk to the user` header = `Event`
      ...
      ( group = `Talk to the user` header = `Message` sub = `MessageBox, Types and Custom Actions` keywords = `confirm warning error success information dialog action` path = `src` app = `z2ui5_cl_smp_app_382` )
      ( group = `Talk to the user` header = `Message` sub = `MessagePopover URL Policy (A)` keywords = `url policy link security validator relative allow deny` path = `src` app = `z2ui5_cl_smp_app_474` )
      ( group = `Talk to the user` header = `Message`
      ...
      ( group = `Talk to the user` header = `Message` sub = `MessageView and MessagePopover (A)` keywords = `messagepopover messageitem dialog grouped message list` path = `src` app = `z2ui5_cl_smp_app_452` )
      ( group = `Talk to the user` header = `Popover` sub = `Basic Example with Placement` keywords = `placement anchor button confirm cancel popover_display` path = `src` app = `z2ui5_cl_smp_app_026` )
      ( group = `Talk to the user` header = `Popover` sub = `Open from a Table Row (A)` keywords = `list report dynamicpage row link details table popover slot cs_view focus control_by_id` path = `src` app = `z2ui5_cl_smp_app_052` )
      ( group = `Talk to the user` header = `Popover` sub = `Open Together with the View Build` keywords = `initial render one roundtrip anchor button` path = `src` app = `z2ui5_cl_smp_app_490` )
      ( group = `Talk to the user` header = `Popover` sub = `QuickView Contact Card` keywords = `quickview contact card links grouped fields` path = `src` app = `z2ui5_cl_smp_app_109` )
      ( group = `Talk to the user` header = `Popover` sub = `Select from a List` keywords = `list selection placement anchor` path = `src` app = `z2ui5_cl_smp_app_081` )
      ( group = `Talk to the user` header = `Popover` sub = `Toggle by ID (toggleBy) (A)` keywords = `toggleby open close control_by_id whitelisted` path = `src` app = `z2ui5_cl_smp_app_465` )
      ( group = `Talk to the user` header = `Popup` sub = `Dialog inside a Dialog` keywords = `nested stack popup in popup second dialog` path = `src` app = `z2ui5_cl_smp_app_161` )
      ( group = `Talk to the user` header = `Popup` sub = `Edit a Row in a Second App` keywords = `table edit row popup app nav_app_call get_app_prev add delete save master detail` path = `src` app = `z2ui5_cl_smp_app_500` )
      ( group = `Talk to the user` header = `Popup` sub = `Element Binding to the Selected Row (A)` keywords = `element binding relative path aggregation dialog row` path = `src` app = `z2ui5_cl_smp_app_470` )
      ( group = `Talk to the user` header = `Popup` sub = `Navigate between Dialogs (NavContainer) (A)` keywords = `navcontainer dialog pages back forward` path = `src` app = `z2ui5_cl_smp_app_170` )
      ( group = `Talk to the user` header = `Popup` sub = `Value Help: Suggestions and F4 Dialog` keywords = `f4 search help suggestion input dialog select` path = `src` app = `z2ui5_cl_smp_app_009` )
      ( group = `Talk to the user` header = `Popup` sub = `Ways to Open a Dialog (A)` keywords = `dialog sub app destroy rerender background view` path = `src` app = `z2ui5_cl_smp_app_012` )
      ( group = `Move through the app` header = `Focus`
        ...
        intro = `More than one view and more than one app: navigation and app calls, views nested inside views, and the small things that decide where the user is looking - focus, scrolling, a timer.`
        path = `src` app = `z2ui5_cl_smp_app_421` )
      ( group = `Move through the app` header = `Focus` sub = `Jump to the Next Input on Enter (A)` keywords = `cursor enter tab next field form set_focus` path = `src` app = `z2ui5_cl_smp_app_189` )
      ( group = `Move through the app` header = `Focus` sub = `Set Focus and Select Text in an Input (A)` keywords = `cursor set_focus selection position textfield` path = `src` app = `z2ui5_cl_smp_app_133` )
      ...
      ( group = `Move through the app` header = `Hash` sub = `Routing Mode FRESH (a New Instance)` keywords = `routing mode fresh default off navigation restart new instance nav_app_call` path = `src` app = `z2ui5_cl_smp_app_468` )
      ...
      ( group = `Move through the app` header = `Nested View` sub = `Embed Another App's View` keywords = `sub app class embed instantiate another app rtti` path = `src` app = `z2ui5_cl_smp_app_104` )
      ( group = `Move through the app` header = `Nested View` sub = `Master-Detail with FlexibleColumnLayout` keywords = `fcl master detail list report two column split` path = `src` app = `z2ui5_cl_smp_app_097` )
      ( group = `Move through the app` header = `Nested View` sub = `Three Columns with FlexibleColumnLayout` keywords = `fcl three column detail detail deep navigation` path = `src` app = `z2ui5_cl_smp_app_098` )
      ( group = `Move through the app` header = `Scroll` sub = `Scroll a Control into View (A)` keywords = `scroll_into_view control id validation jump` path = `src` app = `z2ui5_cl_smp_app_363` )
      ( group = `Move through the app` header = `Scroll` sub = `Scroll to a Pixel Position (A)` keywords = `position pixel scroll_to restore refresh toolbar` path = `src` app = `z2ui5_cl_smp_app_362` )
      ( group = `Move through the app` header = `Timer` sub = `Progress Indicator during a Backend Call (A)` keywords = `progressindicator busy wait long running backend` path = `src` app = `z2ui5_cl_smp_app_064` )
      ( group = `Move through the app` header = `Timer` sub = `Refresh the View Every n Seconds (A)` keywords = `interval polling auto refresh follow_up_action seconds` path = `src` app = `z2ui5_cl_smp_app_028` )
      ...
      ( group = `Reach outside the view` header = `Browser` sub = `Keyboard Layout of an Input (inputmode) (C)` keywords = `inputmode soft keyboard numeric keypad barcode scanner mobile inputext bound property` path = `src` app = `z2ui5_cl_smp_app_516` )
      ( group = `Reach outside the view` header = `Browser` sub = `Local and Session Storage (A,C)` keywords = `localstorage sessionstorage persist store_data offline` path = `src` app = `z2ui5_cl_smp_app_327` )
      ( group = `Reach outside the view` header = `Browser` sub = `Logout from the Client (A)` keywords = `logoff signout icf session end fiori launchpad` path = `src` app = `z2ui5_cl_smp_app_361` )
      ( group = `Reach outside the view` header = `Browser` sub = `Open a URL in a New Tab (A)` keywords = `url window open_new_tab link target` path = `src` app = `z2ui5_cl_smp_app_073` )
      ( group = `Reach outside the view` header = `Browser` sub = `Open Mail, Phone and SMS Links (A)` keywords = `mailto tel sms urlhelper redirect native link` path = `src` app = `z2ui5_cl_smp_app_316` )
      ( group = `Reach outside the view` header = `Browser` sub = `Reload the Page (A)` keywords = `reload refresh restart location_reload url` path = `src` app = `z2ui5_cl_smp_app_492` )
      ( group = `Reach outside the view` header = `Browser` sub = `Scan Field with Submit (InputExt) (A,C)` keywords = `barcode scanner scan field submit enter soft keyboard none inputext focus warehouse` path = `src` app = `z2ui5_cl_smp_app_530` )
      ( group = `Reach outside the view` header = `Browser` sub = `Set the Tab Favicon (A)` keywords = `favicon icon tab image data uri` path = `src` app = `z2ui5_cl_smp_app_491` )
      ( group = `Reach outside the view` header = `Browser` sub = `Set the Tab Title (A)` keywords = `document.title tab caption headline set_title` path = `src` app = `z2ui5_cl_smp_app_125` )
      ( group = `Reach outside the view` header = `Control Behaviour` sub = `Aggregation Item by Index (A)` keywords = `carousel aggregation item index clone template setactivepage positional control_by_id` path = `src` app = `z2ui5_cl_smp_app_514` )
      ( group = `Reach outside the view` header = `Control Behaviour` sub = `Expand a Panel by ID (setExpanded) (A)` keywords = `panel collapse expand setexpanded control_by_id whitelisted` path = `src` app = `z2ui5_cl_smp_app_448` )
      ( group = `Reach outside the view` header = `Control Behaviour` sub = `Inline CSS on a Control (css) (A)` keywords = `css inline style background color opacity control_by_id dom node no property` path = `src` app = `z2ui5_cl_smp_app_513` )
      ( group = `Reach outside the view` header = `Control Behaviour` sub = `MultiInput with Tokens (C)` keywords = `multiinput token tokens suggestion custom control` path = `src` app = `z2ui5_cl_smp_app_078` )
      ( group = `Reach outside the view` header = `Control Behaviour` sub = `Open the PDF Viewer by ID (A)` keywords = `pdfviewer pdf document viewer popup control_by_id whitelisted` path = `src` app = `z2ui5_cl_smp_app_449` )
      ( group = `Reach outside the view` header = `Control Behaviour` sub = `Register an Icon Font (A)` keywords = `icon font registerfont iconpool tnt collection glyph missing control_global` path = `src` app = `z2ui5_cl_smp_app_518` )
      ( group = `Reach outside the view` header = `Control Behaviour` sub = `Switch NavContainer Page by ID (A)` keywords = `navcontainer icontabbar icontabheader page switch control_by_id whitelisted` path = `src` app = `z2ui5_cl_smp_app_088` )
      ( group = `Reach outside the view` header = `Control Behaviour` sub = `The Global Busy Indicator (A)` keywords = `busy indicator global control_global show hide blocking wait spinner long running` path = `src` app = `z2ui5_cl_smp_app_515` )
      ( group = `Reach outside the view` header = `Control Behaviour` sub = `Wizard with Steps (A)` keywords = `wizard step branching discardprogress setnextstep control_by_id` path = `src` app = `z2ui5_cl_smp_app_202` )
      ...
      ( group = `Reach outside the view` header = `Device` sub = `Geolocation from the Browser (C)` keywords = `gps position latitude longitude altitude location` path = `src` app = `z2ui5_cl_smp_app_120` )
      ( group = `Reach outside the view` header = `File` sub = `Download to the Browser (A)` keywords = `export save base64 attachment xstring document` path = `src` app = `z2ui5_cl_smp_app_186` )
      ( group = `Reach outside the view` header = `File` sub = `Upload to the Backend (C)` keywords = `fileuploader base64 attachment import picture document` path = `src` app = `z2ui5_cl_smp_app_074` )
      ( group = `Reach outside the view` header = `File` sub = `Upload with an UploadSet (C)` keywords = `uploadset upload drag drop multiple files base64 attachment uploadsetext companion` path = `src` app = `z2ui5_cl_smp_app_517` ) ).
