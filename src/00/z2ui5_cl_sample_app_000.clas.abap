...
CLASS z2ui5_cl_sample_app_000 IMPLEMENTATION.
  ...
  METHOD get_catalog.
    ...
    result = VALUE #(
      ...
      ( group = `framework - new (beta)` header = `Tree` sub = `drag and drop between folders` app = `z2ui5_cl_demo_app_461` )
      ( group = `framework - new (beta)` header = `Tree` sub = `inside a popup dialog` app = `z2ui5_cl_demo_app_462` )
      ( group = `framework - new (beta)` header = `Tree` sub = `nested model` app = `z2ui5_cl_demo_app_460` )
