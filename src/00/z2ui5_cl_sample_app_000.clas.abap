...
CLASS z2ui5_cl_sample_app_000 IMPLEMENTATION.
  ...
  METHOD get_catalog.
    ...
    result = VALUE #(
      ...
      ( group = `use of built-in popups` header = `basic` sub = `popups and flow` app = `z2ui5_cl_demo_app_341` )
      ( group = `use of built-in popups` header = `CC CameraSelector` sub = `` app = `z2ui5_cl_demo_app_306` )
      ( group = `use of built-in popups` header = `CC Data loss protection` sub = `` app = `z2ui5_cl_demo_app_279` )
      ( group = `use of built-in popups` header = `popups` sub = `p13n Dialog` app = `z2ui5_cl_demo_app_090` )
      ( group = `use of built-in popups` header = `selscreen` sub = `filter bar with variant management WIP` app = `z2ui5_cl_demo_app_111` )
