...
CLASS z2ui5_cl_sample_app_g01 IMPLEMENTATION.
  ...
  METHOD get_catalog.
    ...
    result = VALUE #(
      ...
      ( group = `restricted - experimental` header = `Gantt` sub = `chart with tree table (sap.gantt)` app = `z2ui5_cl_demo_app_076` )
      ( group = `restricted - experimental` header = `Gantt` sub = `relationships between shapes (sap.gantt)` app = `z2ui5_cl_demo_app_179` )
      ...
      ( group = `restricted - experimental` header = `p13n` sub = `personalization popup driven from ABAP` app = `z2ui5_cl_demo_app_090` )
      ...
      ( group = `restricted - experimental` header = `ViewSettingsDialog` sub = `sort, group and filter a table` app = `z2ui5_cl_demo_app_099` )
