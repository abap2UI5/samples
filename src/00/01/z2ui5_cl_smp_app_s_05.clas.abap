...
*      FIELDS icfactive
*      WHERE icf_name = `Z2UI5_APC_SMP_01`
*      INTO @DATA(icfactive).
...
*      page->message_strip(
*          text    = `ICF Service '/sap/bc/apc/sap/z2ui5_apc_smp_01' is not active. WebSocket communication will not work. Please activate the ICF Service in transaction SICF.`
*          type    = `Warning`
*          visible = abap_true ).
*    ENDIF.
    ...
    IF client->check_on_init( ).
      view->_generic( name = `script`
            ...
            `(()=>{ ` &&
            `  const ws_url = (window.location.origin + '/sap/bc/apc/sap/z2ui5_apc_smp_01').replace('http','ws');` &&
            `  try { ` &&
