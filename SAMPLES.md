...
**Every one of them ships on every branch**: `src/` is one flat package, and
nothing in it is stripped from `702`.
...
| Large Table with Growing and ScrollContainer<br>10.000 rows in one table: growing with scroll-to-load, a sticky header toolbar and sorting in the backend.<br><sub>growing 10000 rows sticky toolbar sort performance</sub><br><sub>docs: [cookbook/model/tables](https://abap2ui5.github.io/docs/cookbook/model/tables)</sub> | [`Z2UI5_CL_SMP_APP_006`](src/z2ui5_cl_smp_app_006.clas.abap) |
| Live Search over a Large Table<br>A SearchField that filters a 6000-row table on every keystroke, wired with check_queue_last and check_no_busy so no keystroke is lost and the busy overlay never flashes over the field.<br><sub>live search table filter keystroke roundtrip busy indicator overlay check_queue_last check_no_busy typing</sub><br><sub>docs: [cookbook/model/tables](https://abap2ui5.github.io/docs/cookbook/model/tables)</sub> | [`Z2UI5_CL_SMP_APP_059`](src/z2ui5_cl_smp_app_059.clas.abap) |
| Refused Cell Values (t_model_skipped)<br>What happens to a table cell the user fills with text that does not fit its ABAP type: the cell is skipped, t_model_skipped names it, and the app shows the user why.<br><sub>table edit refused cell conversion error t_model_skipped valuestate packed price integer nested row_parent</sub> | [`Z2UI5_CL_SMP_APP_504`](src/z2ui5_cl_smp_app_504.clas.abap) |
| Search in the Backend (SearchField)<br>A SearchField that searches in the backend on Go or Enter, rather than filtering what was already sent.<br><sub>search go enter server side where</sub><br><sub>docs: [cookbook/model/tables](https://abap2ui5.github.io/docs/cookbook/model/tables)</sub> | [`Z2UI5_CL_SMP_APP_053`](src/z2ui5_cl_smp_app_053.clas.abap) |
| Selection Modes: Single and Multi Select<br>The table selection modes - none, single and multi - side by side, and what each returns.<br><sub>selectionmode none single multi segmentedbutton checkbox</sub><br><sub>docs: [cookbook/model/tables](https://abap2ui5.github.io/docs/cookbook/model/tables)</sub> | [`Z2UI5_CL_SMP_APP_019`](src/z2ui5_cl_smp_app_019.clas.abap) |
...
| Sample | Class |
|---|---|
| Progress Indicator during a Backend Call (A)<br>A ProgressIndicator during a long backend call, driven by follow-up actions rather than by a frozen screen.<br><sub>progressindicator busy wait long running backend</sub><br><sub>docs: [cookbook/browser_interaction/timer](https://abap2ui5.github.io/docs/cookbook/browser_interaction/timer)</sub> | [`Z2UI5_CL_SMP_APP_064`](src/z2ui5_cl_smp_app_064.clas.abap) |
| Refresh the View Every n Seconds (A)<br>Refreshes the view every n seconds - the polling interval as a follow-up action the app renews itself.<br><sub>interval polling auto refresh follow_up_action seconds</sub><br><sub>docs: [cookbook/browser_interaction/timer](https://abap2ui5.github.io/docs/cookbook/browser_interaction/timer)</sub> | [`Z2UI5_CL_SMP_APP_028`](src/z2ui5_cl_smp_app_028.clas.abap) |

### Tree

| Sample | Class |
|---|---|
| Drag and Drop Nodes (A,C)<br>Drag and drop of tree nodes: which node was moved where, and what that does to the hierarchy behind it.<br><sub>dnd move node hierarchy binding context</sub><br><sub>docs: [cookbook/model/trees](https://abap2ui5.github.io/docs/cookbook/model/trees)</sub> | [`Z2UI5_CL_SMP_APP_461`](src/z2ui5_cl_smp_app_461.clas.abap) |
| Editable Nodes with CustomTreeItem (C)<br>Editable tree nodes with a CustomTreeItem, so a rename in the tree writes back into the ABAP hierarchy.<br><sub>customtreeitem rename input binding write back</sub><br><sub>docs: [cookbook/model/trees](https://abap2ui5.github.io/docs/cookbook/model/trees)</sub> | [`Z2UI5_CL_SMP_APP_463`](src/z2ui5_cl_smp_app_463.clas.abap) |
| Inside a Dialog (C)<br>A tree inside a dialog, including which nodes stay expanded when the popup is opened again.<br><sub>popup expand state hierarchy nodes</sub><br><sub>docs: [cookbook/model/trees](https://abap2ui5.github.io/docs/cookbook/model/trees)</sub> | [`Z2UI5_CL_SMP_APP_462`](src/z2ui5_cl_smp_app_462.clas.abap) |
| Nested ABAP Table in a sap.m.Tree<br>A nested ABAP table rendered as a sap.m.Tree - the hierarchy comes from the data, not from the view.<br><sub>hierarchy nodes nested json items</sub><br><sub>docs: [cookbook/model/trees](https://abap2ui5.github.io/docs/cookbook/model/trees)</sub> | [`Z2UI5_CL_SMP_APP_460`](src/z2ui5_cl_smp_app_460.clas.abap) |
