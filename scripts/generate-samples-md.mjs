...
/* The two counts below go to 1 and to 0 — `src/00/98` was emptied down to a
 * single app on 2026-09-22 (AGENTS.md section 2) and the shared data objects
 * went with it. A sentence that reads "The other 1 are" or offers to list
 * "0 further classes" is a generated sentence about nothing, so both bend to
 * what the tree actually holds. */
const systemIntro = system.length === 1
  ? 'One app with no tile anywhere'
  : `${system.length} apps with no tile anywhere`;
const systemLine = systemApps === 1
  ? 'The other one is under `src/00` and is'
  : `The other ${systemApps} are under \`src/00\` and are`;
const dataObjectLine = dataObjects === 0
  ? ''
  : ` ${dataObjects} further ${dataObjects === 1 ? 'class carries' : 'classes carry'} the sample name and`
    + ` ${dataObjects === 1 ? 'is a data object' : 'are data objects'} the samples share rather than apps;`
    + ' they are listed too, so nothing in the tree is invisible.';
...
\`src/01\` plus this overview app. ${systemLine}
stripped from \`702\`.${dataObjectLine}
...
${systemIntro} (AGENTS.md section 3) — reachable
by class name only, which is what this section is for. ${system.length === 1 ? 'It is' : 'They are'} held to a
