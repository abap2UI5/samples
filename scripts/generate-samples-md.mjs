...
const { tiles: basics, hidden } = scanSamples();

/* How many APPS are here, which is a different question from how many classes
 * match the sample naming scheme: a class can carry the name and be a data
 * object rather than an app. The `isApp` flag comes from the source
 * (lib/scan-samples.mjs), and the count is rendered as `dataObjects` below
 * rather than written out here, where it would go stale.
 *
 * Every app in the tree ships on every branch since `src/` was flattened
 * (AGENTS.md section 1): the samples, the helper apps they call, and the
 * overview app itself, which is not a tile and therefore not in the scan. */
const isApp = (t) => t.isApp;
const apps = [...basics, ...hidden].filter(isApp).length + 1;
const dataObjects = [...basics, ...hidden].filter((x) => !isApp(x)).length;

// --- the samples, by category ---------------------------------------------
...
/* The count below goes to 0 — the last class that carried the sample name
 * without being an app went with `src/00` on 2026-09-22 (AGENTS.md section 1).
 * A sentence offering to list "0 further classes" is a generated sentence
 * about nothing, so it bends to what the tree actually holds. */
...
nothing in it is stripped from \`702\`.${dataObjectLine}
...
one idea. With the ${apps - basics.length - 1} helper apps they call and the
overview app itself, that is ${apps} apps on every branch.
...
for (const tile of [...basics, ...hidden]) {
...
if (Number(claim[1]) !== basics.length) {
  throw new Error(
    ...
    + '(the catalogued samples) — fix the README',
  );
}
